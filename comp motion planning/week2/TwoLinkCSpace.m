...
video=1;
if video
  video_writer = VideoWriter('torus_plan.avi', 'Uncompressed AVI');
  open(video_writer);
end
h1 = figure;
...
axis (sz*[-1 1 -1 1]);
for i = 1:n
    fv1 = TwoLinkRobot ([x(i), y(i)]);
    p.Vertices = fv1.vertices;
...
%     h.YData = y(i);
    ...
    drawnow;
    if video
        writeVideo(video_writer, getframe(h2));
    end
end
if video
  close(video_writer);
